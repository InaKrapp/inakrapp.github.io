...
```
```{r}
sum(household_data$household_income)
```
...
```
```{r}
lm1 <- lm(household_income ~ household_names, data = household_data)
summary(lm1)
```
...
To finish this first half, we'll look at packages and what you can do with them.
...
```
```{r}
summary(lm(body_mass_g ~ sex + year, data = penguins))
```
